 ...
 * package com.test.multithreading;
 * 
 * public class PrintSequenceRunnable implements Runnable{ public int
 * PRINT_NUMBER_UPTO=10; static int number = 1; int remainder; static Object
 * lock = new Object();
 * 
 * PrintSequenceRunnable(int remainder) { this.remainder = remainder; }
 * 
 * @Override public void run() { while(number<PRINT_NUMBER_UPTO) { synchronized
 * (lock) { while(number%2 != remainder) {//wait for number other than remainder
 * try { lock.wait(); }catch(InterruptedException e) { e.printStackTrace(); } }
 * System.out.println(Thread.currentThread().getName() +" " + number); number++;
 * try { Thread.sleep(1000); }catch(InterruptedException e) {
 * e.printStackTrace(); } lock.notifyAll(); } } }
 * 
 * public static void main(String[] args) { PrintSequenceRunnable runnable1 =
 * new PrintSequenceRunnable(0); PrintSequenceRunnable runnable2 = new
 * PrintSequenceRunnable(1); Thread t1 = new Thread(runnable1,"even"); Thread t2
 * = new Thread(runnable2,"odd"); t1.start(); t2.start();
 * 
 * }
 * 
 * 
